...
To convert your data into a genlight object you can arrange your data set in the same format and also attach already meta data for individuals (will be stored in the \@other slot under data), coordinates as lat longs and population structure.
...
Some of the function require additional meta data, e.g. the gl2fasta need sequnce data (TrimmedSequnce), the position of the SNP in the sequence for each loci ```@position``` and the type of allelels for each loci ```@loc.all```. If such data is available they information needs to be stored in the correct slots to work with the function. The code below exports our genlight object to a fasta format.
...
And finally we can use the ```gl2fasta``` function (using method 1, see explanation via ?gl2fasta):
