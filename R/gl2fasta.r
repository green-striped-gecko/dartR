    ...
    for (i in 1:r) {
      for (j in 1:c) {
        ...
        if(method==2){
          ...
          seq[j] <- stringr::str_sub(trimmed, start=(snpos), end=(snpos))
        }
        ...
        if (matrix[i,j] == 2 && !is.na(matrix[i,j])) {
          ...
          snpbase <- stringr::str_sub(trimmed, start=(snpos), end=(snpos))
          ...
          if(method==2){
            ...
            seq[j] <- stringr::str_sub(target, start=1, end=nchar(trimmed))
          }else if(method==4){
            seq[j] <- snpbase
          }
        }
        ...
        if (is.na(matrix[i,j])) {
          seq[j] <- "N"
          if(method==2){
            seq[j] <- stringr::str_pad(seq[j], nchar(trimmed), side = c("right"), pad = "N")
          }
        }
      }
      #setTxtProgressBar(pb, i/r)
