...
#'
#' @param x -- either a genlight object containing a column named "TrimmedSequence" containing the sequence of the SNPs (the sequence tag) trimmed of adapters as provided by DArT; or a path to a fasta file with the query sequences [required].
#' @param ref_genome -- path to a reference genome in fasta of fna format [required].
#' @param task -- four different tasks are supported: 1.) “megablast”, for very similar sequences (e.g, sequencing errors), 2.) “dc-megablast”, typically used for inter-species comparisons, 3.) “blastn”, the traditional program used for inter-species comparisons, 4.) “blastn-short”, optimized for sequences less than 30 nucleotides [default: "megablast"].
#' @param Percentage_identity --  not a very sensitive or reliable measure of sequence similarity however it is a reasonable proxy for evolutionary distance. The evolutionary distance associated with a 10% change in percent identity is much greater at longer distances. Thus, a change from 80 – 70% identity might reflect divergence 200 million years earlier in time, but the change from 30% to 20% might correspond to a billion year divergence time change [default: 70].
#' @param Percentage_overlap -- calculated as alignment length divided by the query length or subject length (whichever is shortest of the two lengths, i.e.  length / min(qlen,slen) ) [default: 0.8]
#' @param bitscore -- a rule-of-thumb for inferring homology, a bit score of 50 is almost always significant [default: 50]
#' @param number_of_threads -- number of threads (CPUs) to use in blastn search [default: 2]
...
#' \itemize{
#' \item “megablast”, for very similar sequences (e.g, sequencing errors)
#' \item “dc-megablast”, typically used for inter-species comparisons
#' \item “blastn”, the traditional program used for inter-species comparisons
#' \item “blastn-short”, optimized for sequences less than 30 nucleotides
#' }
#'
...
#'
#' @return If the input is a genlight object: returns a genlight object with one hit per sequence merged to the slot $other$loc.metrics and the file names of databases containing unfiltered aligned sequences, filtered aligned sequences and one hit per sequence that were saved in the tempdir stored in the slot other$history. If the input is a fasta file: returns a dataframe with one hit per sequence.
#'
#' @export
#'
#' @author Berenice Talamantes Becerra & Luis Mijangos (Post to \url{https://groups.google.com/d/forum/dartr})
#'
#' @examples
#' \dontrun{
...
#' }
#'
#'@references Altschul, S. F., Gish, W., Miller, W., Myers, E. W., & Lipman, D. J. (1990). Basic local alignment search tool. Journal of molecular biology, 215(3), 403-410.
#'
#'@references Altschul, S. F., Madden, T. L., Schäffer, A. A., Zhang, J., Zhang, Z., Miller, W., & Lipman, D. J. (1997). Gapped BLAST and PSI-BLAST: a new generation of protein database search programs. Nucleic acids research, 25(17), 3389-3402.
#'
#'@references Pearson, W. R. (2013). An introduction to sequence similarity (“homology”) searching. Current protocols in bioinformatics, 42(1), 3-1.
#'
...
gl.blast <- function(x,
  ...
  # ratio of the alignment length divided by the query length or subject length (whichever is shortest of the two lengths)
  min_length_BF <-
  ...
  blast_res_filtered <-
    blast_res_unfiltered[which(
      blast_res_unfiltered$PercentageOverlap >= Percentage_overlap &
        blast_res_unfiltered$bitscore >= bitscore
    ),]
  # splitting hits by sequence
  all_hits <-
    split(x = blast_res_filtered, f = blast_res_filtered$qseqid)
  # ordering by first considering the highest percentage identity, then the highest percentage overlap, then the highest bitscore. Only one hit per sequence is kept based on these selection criteria.
  one_hit_temp <- lapply(all_hits, function(x) {
    x[order(x$pident, x$PercentageOverlap, x$bitscore, decreasing = T),][1,]
  })
  ...
  if (class(x)[1] == "genlight") {
    ...
    x$other$loc.metrics <-
      merge(one_hit_temp, one_hit, by = "qseqid", all = T)
  }
  ...
  # creating file names
  temp_blast_unfiltered <-
    tempfile(pattern = "temp_blast_unfiltered_")
