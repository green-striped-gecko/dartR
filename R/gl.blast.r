...
gl.blast <- function(x, ref_genome, task = "megablast", 
  ...
  if (class(x)[1] == "genlight") {
    ...
    write.table(fasta.input, file = paste0(tempdir(), 
                                           "/fasta.input"), quote = F, row.names = F, 
                col.names = F)
  } else {
    file.copy(from = x, to = paste0(tempdir(), 
                                    "/fasta.input"), overwrite = T)
  }
  ...
  if (all(path_makeblastdb == "")) {
    stop(error("Executable for makeblastdb not found! Please make sure that the software\n is correctly installed.\n\n"))
  }
  
  # creating BLAST databases
  system(paste(path_makeblastdb, "-in", ref_genome, 
               "-dbtype", "nucl", "-out", paste0(tempdir(), 
                                                 "/db_blast")))
  ...
  ## if windows
  if (!grepl("unix", .Platform$OS.type, ignore.case = TRUE)) {
    path_blastn <- tryCatch(system(sprintf("where %s", 
                                           "blastn"), intern = TRUE)[1], warning = function(w) "", 
                            error = function(e) "")
    if (grepl("\\s", path_blastn)) {
      stop(error("The path to the executable for blastn has spaces. Please move it to a\n path without spaces so BLAST can work.\n\n"))
    }
  }
  ## if not found
  if (all(path_blastn == "")) {
    stop(error("Executable for blastn not found! Please make sure that the software is\n correctly installed.\n\n"))
  }
  ...
  # BLASTing
  system(paste(path_blastn, " -task ", task, " -db ", 
               paste0(tempdir(), "/db_blast"), " -query ", 
               paste0(tempdir(), "/fasta.input"), " -out ", 
               paste0(tempdir(), "/output_blast.txt"), " -perc_identity ", 
               Percentage_identity, " -num_threads ", number_of_threads, 
  ...
  if (file_size > 0) {
    blast_res_unfiltered <- read.table(file = paste0(tempdir(), 
  ...
  colnames(blast_res_unfiltered) <- c("qseqid", "sacc", 
                                      "stitle", "qseq", "sseq", "nident", "mismatch", 
                                      "pident", "length", "evalue", "bitscore", "qstart", 
                                      "qend", "sstart", "send", "gapopen", "gaps", 
                                      "qlen", "slen")
  
  # calculate percentage overlap ratio of the
  # alignment length divided by the query length or
  # subject length (whichever is shortest of the two
  # lengths)
  ...
  # filtering first by percentage overlap and
  # bitscore
  ...
  # ordering by first considering the highest
  # percentage identity, then the highest percentage
  # overlap, then the highest bitscore. Only one hit
  # per sequence is kept based on these selection
  # criteria.
  one_hit_temp <- lapply(all_hits, function(x) {
    x[order(x$pident, x$PercentageOverlap, x$bitscore, 
            decreasing = T), ][1, ]
  })
