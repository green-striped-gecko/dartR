...
#' populations based on its proximity to each population defined by a confidence
#' ellipse.
#'
...
#'  limits are no longer considered.
#' \item The ordination is repeated on the remaining populations to again yield
#' a series of orthogonal (independent) axes, a necessary condition for
#' combining likelihoods calculated from each axis.
#' \item A workable subset of dimensions is chosen, normally equal to the number
#'  of dimensions with substantive eigenvalues.
#' \item The log-likelihood of the value for the unknown on each axis is
#' calculated, weighted by the eigenvalue for that axis, and summed over all
#' dimensions as an assignment weighting. The assignment weighting is calculated
#' for a point on the boundary of the 95% (or as specified) confidence
#' envelope, and an index is calculated as the ratio of the two. A value less
#' than 1 indicates that the unknown is within the confidence envelope, a value
#' greater than 1 indicates that the unknown lies outside the confidence
#' envelope. A smaller value of the index indicates greater conformity of the
#' unknown genotype with the target population.
#' }
#' @details
#' There are three considerations to the assignment. First, consider only those
...
#' envelope. This second step is good for eliminating populations from
#' consideration, but does not provide confidence in assignment.
#'
#' The third step is to consider the assignment probabilities. This approach
#' calculates the squared Generalised Linear Distance (Mahalanobis distance) of
#' the unknown from the centroid for each population, and calculates the
#' probability associated with its quantile under the zero truncated normal
#' distribution. This index takes into account position of the unknown in
#' relation to the confidence envelope in all selected dimensions of the
#' ordination. An index is calculated based on the distance of the unknown from
#' the population centroid, scaled for the distance of the standardized
#' confidence ellipse. An index of less than 1 indicates that the unknown lies
#' within the confidence ellipse for the focal population; an index of greater
#' than 1 indicates that the unknown lies outside the confidence ellipse for the
#' focal population. The smaller the index, the greater the confidence in the
#'  assignment.
#'
#'   Wt is a weighted log-likelihood
#'   CE is the value of the Index on the boundary of the 95 % confidence
#'   envelope
#'   Index is the position of the unknown relative to the boundary of the 95 %
#'   confidence envelope
#'      An index value less than 1 indicates the unknown resides inside the
#'      confidence ellipse for the focal population
#'      An index value greater than 1 indicates the unknown resides outside the
#'       confidence ellipse for the focal population
#'
...
#' population 'unknown') and #' populations for which the focal individual is
#'  not distinctive (number of loci with private alleles less than or equal to
#'  thresold t.
#'
...
#'
# @examples Test run with a focal individual from the Macleay River (EmmacMaclGeor) x <- gl.assign.pa(testset.gl, unknown='UC_00146',
# nmin=10, threshold=1,verbose=3) x <- gl.assign.pca(x, unknown='UC_00146', alpha=0.05,verbose=3)

gl.assign.pca2 <- function(x,
                           unknown,
                           plevel = 0.95,
                           verbose = NULL) {
    # SET VERBOSITY
    ...
    # Determine the number of dimensions for confidence envelope (the ordination
    #and dimension reduction) From the eigenvalue distribution
    s <- sum(pcoa$eig)
    ...
    if (verbose >= 2) {
        cat(report(
            "    Dimension of confidence envelope set at",
            dimension,
            "\n"
        ))
    }
    ...
    for (i in popNames(x)) {
        ...
        count <- count + 1
        A <-
            pcoa$scores[(df$pop == i |
                             df$pop == "unknown"), ]  # Including the unknown to be conservative
        mu <- colMeans(A)
        sigma <- stats::cov(A)
        # testset <- rbind(pcoa$scores[df$pop=='unknown',],A) row.names(testset)[1] <- 'unknown'
        transform <- SIBER::pointsToEllipsoid(A, sigma, mu)
        # transform2 <- SIBER::pointsToEllipsoid(pcoa$scores[df$pop=='unknown',], sigma, mu)
        inside.or.out <-
            SIBER::ellipseInOut(transform, p = plevel)
        result[count, 1] <- i
    ...
    if (verbose >= 3) {
        ...
        if (all(result$hit == TRUE)) {
            ...
            cat(
                "  Populations elimated from consideration:",
    ...
    if (nInd(x2) < 2) {
        ...
        best <-
            as.character(df$Population[df$Assign == "yes"][1])
        # cat(' Wt is a weighted log-likelihood\n') cat(' CE is the value of the Index on the boundary of the',alpha*100,'% confidence
