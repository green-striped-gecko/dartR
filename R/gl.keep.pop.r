#' @name gl.keep.pop
#' @title Remove all but specified populations from a genelight \{adegenet\} object
#' @description
...
# FUNCTION SPECIFIC ERROR CHECKING
  ...
  if(is.null(as.pop)){
    if (is.null(pop(x)) | is.na(length(pop(x))) | length(pop(x)) <= 0) {
      stop(error("Fatal Error: Population assignments not detected, run gl.compliance.check() and revisit population assignments\n"))
    }
  }
  ...
  if (!is.null(as.pop)){    
    if(as.pop %in% names(x@other$ind.metrics)){
      ...
      cat(warn("  Warning: individual metric assigned to 'pop' does not exist. Running compliance check\n"))
...
# DO THE JOB
  ...
  if (verbose >= 2) {
    cat("  Retaining only populations", paste(pop.list,collapse=", "),"\n")
  }
...
# REPORT A SUMMARY
  ...
  # Reassign the initial population list if as.pop is specified
    ...
    if (!is.null(as.pop)){
      pop(x) <- pop.hold
      if (verbose >= 2) {cat(report("  Resetting population assignments to initial state\n"))}
    }
