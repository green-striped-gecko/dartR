...
#' x2. 
#' 
#'\strong{Hint:} in case you want to run comparisons between individuals (assuming 
#' individual names are unique), you can simply redefine your population names 
#' with your individual names, as below:
#' 
...
#'   
#' @return A data.frame. Each row shows, for each pair of populations the number of
#'  individuals in each population, the number of loci with fixed differences (same
#'   for both populations) in pop1 (compared to pop2) and vice versa. Same for 
#'   private alleles and finally the absolute mean allele frequency difference 
#'   between loci (AFD).
#'
...
# DO THE JOB
  ...
  pall <- data.frame(p1=pc[,1], p2=pc[,2], pop1=names(pops)[pc[,1]], pop2=names(pops)[pc[,2]], N1=NA, N2=NA,fixed=NA, priv1=NA, priv2=NA, totalpriv=NA, AFD=NA)
  ...
  if(plot.out){
    ...
    for (i in 1:nrow(pall)) mm[pall[i,1], pall[i,2]] <- pall$priv2[i]
    for (i in 1:nrow(pall)) mm[pall[i,2], pall[i,1]] <- pall$priv1[i]
    ...
    data_long <- tibble::rownames_to_column(data,'source')
    data_long <- tibble::as_tibble(data_long) 
    data_long <- pivot_longer(data_long,-source, 'target')
    ...
    nodes <- tibble::tibble(name = unique(c(data_long$source, data_long$target)),
  ...
  if(method=="one2rest"){
    ...
    colnames(pas) <- c("p1","p2","pop1","pop2","N1","N2","fixed","priv1","priv2","totalpriv","AFD")
    for (y in 1:nPop(x)){
      gl2  <- x
      pop(gl2) <- factor(ifelse(pop(gl2)==popNames(x)[y],popNames(x)[y], 'zzest'))
      pops <- seppop(gl2)
      pc <- t(combn(length(pops),2))
      pall <- data.frame(p1=pc[,1], p2=pc[,2], pop1=names(pops)[pc[,1]], pop2=names(pops)[pc[,2]], N1=NA, N2=NA,fixed=NA, priv1=NA, priv2=NA, totalpriv=NA, AFD=NA)
