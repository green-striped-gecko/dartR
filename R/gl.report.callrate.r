...
gl.report.callrate <- function(x, method = "loc", plot_theme = theme_dartR(), plot_colours = wes_palette_Zissou1, verbose = NULL) {
    ...
    ########### FOR METHOD BASED ON LOCUS get title for plots
    if (method == "loc") {
        if (all(x@ploidy == 2)) {
        ...
        p1 <- ggplot(callrate, aes(y = callrate)) + geom_boxplot(color = plot_colours(6)[1], fill = plot_colours(6)[2]) + 
            coord_flip() + plot_theme + xlim(range = c(-1, 1)) + ylim(0, 1) + ylab(" ") + theme(axis.text.y = element_blank(), 
            axis.ticks.y = element_blank()) + ggtitle(title1)

        p2 <- ggplot(callrate, aes(x = callrate)) + geom_histogram(bins = 50, color = plot_colours(6)[1], fill = plot_colours(6)[2]) + 
            coord_cartesian(xlim = c(0, 1)) + xlab("Call rate") + ylab("Count") + plot_theme

        # Print out some statistics
        cat("  Reporting Call Rate by Locus\n")
        cat("  No. of loci =", nLoc(x), "\n")
        cat("  No. of individuals =", nInd(x), "\n")
        cat("    Minimum Call Rate: ", round(min(x@other$loc.metrics$CallRate), 2), "\n")
        ...
        retained <- unlist(lapply(quantile_res, function(y) {
            res <- length(callrate$callrate[callrate$callrate >= y])
        }))
        pc.retained <- round(retained * 100/nLoc(x), 1)
        filtered <- nLoc(x) - retained
        pc.filtered <- 100 - pc.retained
        df <- data.frame(as.numeric(sub("%", "", names(quantile_res))), quantile_res, retained, pc.retained, filtered, 
            pc.filtered)
    ...
    if (method == "ind") {
        ...
        # Boxplot
        p1 <- ggplot(ind.call.rate, aes(y = ind.call.rate)) + geom_boxplot(color = plot_colours(6)[1], fill = plot_colours(6)[2]) + 
            coord_flip() + plot_theme + xlim(range = c(-1, 1)) + ylim(0, 1) + ylab(" ") + theme(axis.text.y = element_blank(), 
            axis.ticks.y = element_blank()) + ggtitle(title1)

        # Histogram
        p2 <- ggplot(data.frame(ind.call.rate), aes(x = ind.call.rate)) + geom_histogram(bins = 50, plot_colours(6)[1], 
            fill = plot_colours(6)[2]) + coord_cartesian(xlim = c(0, 1)) + xlab("Call rate") + ylab("Count") + plot_theme
        ...
        retained <- unlist(lapply(quantile_res, function(y) {
            res <- length(ind.call.rate$ind.call.rate[ind.call.rate$ind.call.rate >= y])
        }))
