...
########### FOR METHOD BASED ON LOCUS
  ...
  if (method == "loc") {
    ...
    p1 <- ggplot(callrate, aes(y = callrate)) + 
      ...
      coord_flip() + 
      plot_theme + xlim(range = c(-1, 1)) + ylim(0,1) + ylab(" ") + 
      theme(axis.text.y = element_blank(), axis.ticks.y = element_blank()) + ggtitle(title1)
    
    p2 <- ggplot(callrate, aes(x = callrate)) + 
      geom_histogram(bins = 50, color = plot_colours[1],fill = plot_colours[2]) + 
      coord_cartesian(xlim = c(0,1)) + xlab("Call rate") + ylab("Count") + 
      plot_theme
...
########### FOR METHOD BASED ON INDIVIDUAL
  ...
  if (method == "ind") {
    ...
    p2 <- ggplot(ind.call.rate, aes(x = ind.call.rate)) + 
      geom_histogram(bins = 50, color = plot_colours[1], fill = plot_colours[2]) + coord_cartesian(xlim = c(0,1)) + 
