...
#' 
#' If plot.out = TRUE, two smear plots (pre-randomisation and post-randomisation)
#' are presented using a random subset of individuals (10) and loci (100) to 
...
gl.random.snp <- function(x,    
  ...
  x@gen <- lapply(1:nrow(snp_matrix), function(i) new("SNPbin", as.integer(snp_matrix[i, ])))
  
  random_snps <- random_snps[order(random_snps)]
    
  if(verbose==5){
  cat(report(paste("The loci that were changed are:",paste(random_snps,collapse=", "),"\n")))
  }
