...
#' cannot be switched off currently).
...
#' @references 
#' \itemize{
#' \item Pritchard, J.K., Stephens, M., Donnelly, P. (2000) Inference of population structure using multilocus genotype data. Genetics 155, 945-959.
#' \item Archer, F. I., Adams, P. E. and Schneiders, B. B. (2016) strataG: An R package for manipulating, summarizing and analysing population genetic data. Mol Ecol Resour. doi:10.1111/1755-0998.12559
#' }
...
gl.run.structure    <- function(x, 
  ...
  if (!requireNamespace("strataG", quietly = TRUE)){
    ...
    if (is.na(oncran)) {
      cat(warn("Package strataG needs to be installed. It is currently not on CRAN, hence you need to install it manually via Github using devtools:\n  devtools::install_github('EricArcher/strataG')\n. Do you want to install strataG from github now? (Y/N)+Enter\n"))
      line <- readline()
      if (substr(line,1,1)=="Y") {
        ...
        devtools::install_github('EricArcher/strataG')
          ...
          stop(error("Package strataG is necessary to run structure. Check the help page for references ?gl.run.structure. Aborting now."))
        }
      }
    } else {
  ...
  if (datatype!="SNP"){
    stop(error("You need to provide a SNP genlight object (ploidy=2)!"))
  } 
...
# DO THE JOB
  ...
  temp_plot <- tempfile(pattern =paste0("Plot",paste0(nmc,"_",mc,collapse = "_")))
