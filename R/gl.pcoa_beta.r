...
#' the distances between the entities in the ordinated space will not exactly represent the distances in the input matrix (stress will be non-zero). This 
#' source of stress will be evident as negative eigenvalues in the deeper dimensions. 
#' 
...
#' if there are missing values, then even a Euclidean distance matrix will not necessarily be 'positive definate'. It follows that some of the eigenvalues 
...
#' loss of data. An alternative is given in a paper "Honey, I shrunk the sample covariance matrix" and more recently by Ledoit and Wolf (2018), but the 
...
#' explain more than the original variables (loci) on average. That is, only informative axes are displayed. The second graph shows the distribution of 
#' eigenvalues for the remaining uninformative (noise) axes, including those with negative eigenvalues. Action is required if the negative eigenvalues are
#' dominant, their sum approaching in magnitude the eigenvalues for axes selected for the final visual solution.
#' 
#' Output is a pcoa object conforming to the object generated by ape:pcoa.
#' 
#' @references
...
#' @return An object of class pcoa containing the eigenvalues, factor scores and other values as specified by ?ape::pcoa
#' @export
#' @author Arthur Georges (Post to \url{https://groups.google.com/d/forum/dartr})
#' @examples
#' pcoa <- gl.pcoa(testset.gl)

# Last amended 11-Aug-19

gl.pcoa.beta <- function(x, verbose=2) {
...
# PLOT DIAGNOSTICS
    ...
    par(mai=c(1,1,0.5,0.5))
      ...
      if (any(eig.raw < 0)){
          main <- "Noise Axes -- some eigenvalues < 0"
      } else {
