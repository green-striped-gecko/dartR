...
#' @description
#' This script draws upon the standard R shape palette to extract a vector of shapes for plotting, where
#' the script that follows has a shape parameter expecting a vector of shapes.
#' @details
#' 
#' By default the shape palette will be displayed in full in the graphics window from which shapes can be selected
#' in a subsequent run, and the vector of shapes returned for later use.
#' 
...
#' # EXAMPLES
#' shapes <- gl.select.shapes() # Will select and display all available shapes
#' shapes <- gl.select.shapes(select=c(1,1,1,5,8)) # Will select and display a restricted set of shapes
#' shapes <- gl.select.shapes(gl=gl,select=c(1,1,1,5,8)) # Will select a restricted set of shapes and check with no. of pops.
#' 
...
gl.select.shapes <- function(gl=NULL,
  ...
  if(is.null(select)){
    if(verbose >= 2){cat(warn("  Warning: Requires shapes not specified, displaying and returning all available 26 shapes\n"))}
    select <- c(seq(1:26)-1) 
    nshapes <- 26
  } else {
    if(min(select < 0 | max(select > 25))){
      stop(error("Fatal Error: specified shapes must be in the range 0-25\n"))
    }
  ...
  # DO THE JOB
    ...
    x <- c(rep(1:5,5),6)
    x <- x[1:length(select)]
    plot(x, y, pch = select, cex=1.5, ylim=c(1,5.5), xlim=c(1,6.5), 
         axes=FALSE, xlab="", ylab="", bg="blue")
    text(x, y, labels=select, pos=3)
    
    if(verbose >= 2){
      cat(report("  Displaying and returning shapes",paste(select,collapse=", "),"\n"))
    }
