...
#' @description
#' Different causes may be responsible for lack of Hardy-Weinberg proportions. 
#' This function helps diagnose potential problems.
...
#'  \enumerate{
#'  \item A histogram with the distribution of p-values of the HWE tests. 
#'  The distribution should be roughly uniform across equal-sized bins.
#'  \item A bar plot with observed and expected (null expectation) number of
#'   significant HWE tests for the same locus in multiple populations (that is, 
#'   the x-axis shows whether a locus results significant in 1, 2, ..., n 
#'   populations. The y axis shows the number of non-significant tests). If HWE 
#'   tests are significant by chance alone, observed and expected number of HWE 
#'   tests should have roughly a similar distribution. 
#'   \item A table where the number of observed and expected significant HWE 
#'   tests are reported by each population, indicating whether these are due to 
#'   heterozygosity excess or deficiency. These can be used to have a clue of 
#'   potential problems (e.g. deficiency might be due to a Wahlund effect, 
#'   presence of null alleles or non-random sampling; excess might be due to sex
#'    linkage or different selection between sexes, demographic changes or small
#'     Ne. See Table 1 in Wapples 2015). The last two columns of the table 
#'     generated by this function report chisquare values and their associated 
...
gl.hwe.diagnostics <- function(x, 
  ...
  p1 <-   ggplot(hweout, aes(Prob)) + 
    geom_histogram(bins = bins,color = colors_hist[1],fill = colors_hist[2]) +
    geom_hline(aes(yintercept = nrow(hweout)/ (bins-1),
