...
#' @param method -- Method: "random", in which case the loci are sampled at random; or avgPIC, in which case the top n loci
...
# FUNCTION SPECIFIC ERROR CHECKING
  ...
  if(mono.rm){
    if(x@other$loc.metrics.flags$monomorphs==FALSE){
      ...
      if(verbose >= 2){cat(report("  Zero monomorphic loci, none deleted\n"))}
    }  
  }
  # Check monomorphs have been removed
  if (x@other$loc.metrics.flags$monomorphs == FALSE){
    if (verbose >= 2){
      cat(warn("  Warning: Dataset contains monomorphic loci which will be included in the",funname,"selections\n"))
    }  
  }
  
  if(!method=='PIC' & !method=='random'){
    if(verbose >= 1){cat(warn("  Warning: parameter method must be set to PIC or random, set to random\n"))}
    method <- "random"
  }
  
  if(n <= 0 | n > nLoc(x)){
    stop(error("Fatal Error: subsample size must be a postive integer >= 1 or <=",nLoc(x),"\n"))
  }

# DO THE JOB
  
  if(method=="random") {
    if (verbose>=2){cat(report("  Subsampling at random",n,"loci from",class(x),"object","\n"))}
