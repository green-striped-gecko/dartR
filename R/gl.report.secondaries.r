...
#' @title Report loci containing secondary SNPs in sequence tags and calculates
...
# FUNCTION SPECIFIC ERROR CHECKING
  ...
  if(isFALSE("AlleleID" %in% names(x$other$loc.metrics)) &
    ...
    stop(error("Neither CloneID or AlleleID metrics were found in the slot loc.metrics, which are required for this function to work\n"))
      }
...
# DO THE JOB
  ...
  if("TrimmedSequence" %in% names(x$other$loc.metrics)) {
    ...
    n.inv.gen <- proc.data[data.table(unique(CloneID)), sum(n.invariant), mult="first"]
    # the mean length of the sequenced tags
    mean.len.tag <- proc.data[data.table(unique(CloneID)), mean(lenTrimSeq), mult="first"]
  } else {
    mean.len.tag <- taglength
    proc.data[, n.variant := .N, by=CloneID]
    # The mean number of SNPs for each tag
    mean.nSNP.tag <- proc.data[data.table(unique(CloneID)), mean(n.variant), mult="first"]
    #                                             The number of tags
    n.inv.gen <- round((mean.len.tag - mean.nSNP.tag) * proc.data[, length(unique(CloneID))], 0)
    cat(warn(paste("The column 'TrimmedSequence' was not found in loc.metrics\n",
                  "Mean tag length assumed to be",taglength,"\n")))
  }
  if (verbose >= 2) {
    cat(report("Counting ....\n"))
  }
...
# Identify secondaries in the genlight object
  ...
  if (is.na(table(duplicated(b))[2])) {
    cat("    Number of secondaries: 0 \n")
      ...
      if(verbose >= 3){
        cat(report(" Tabular 1 to K secondaries (refer plot)\n",table(as.numeric(table(b))),"\n"))
        }
  }  
