...
#' 
#' Colours of node symbols are those of the rainbow.
#' 
#' 
#'@param G -- a G relatedness matrix generated by gl.grm 
...
#'@param node.label.color -- Colour of the text of the node labels [default: "black"]
...
gl.grm.network <- function(G, method="fr", node.size=3, node.label=FALSE, node.label.size=0.7, node.label.color="black", alpha=0.995, title="Network based on G-matrix of genetic relatedness", v=3){
  ...
  colours = rainbow(nlevels(pop(x)))
  my_colours <- colours[pop(x)]
  ...
  if (node.label) {
    ...
    legend("bottomleft", legend=levels(pop(x))  , col = colours , bty = "n", pch=20 , pt.cex = 3, cex = 1, text.col=colours , horiz = FALSE, inset = c(0.1, 0.1))
