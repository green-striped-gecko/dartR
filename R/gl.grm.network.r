...
gl.grm.network <- function(G, 
  ...
  p1 <- ggplot() + 
    ...
    geom_point(data=plotcord,aes(x=X1, y=X2,color=pop),size=node.size) +
    coord_fixed(ratio = 1) +
    theme_void()+
    ggtitle(paste(title,"\n[",layout.name,"]")) +
  ...
  if(node.label==T){
    p1 <- p1 + geom_text(data=plotcord,aes(x=X1, y=X2,label = label.node),size=node.label.size,show.legend=FALSE,color=node.label.color) 
  }
