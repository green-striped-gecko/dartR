...
#' @title Create a proforma recode_ind file for reassigning individual (=specimen) names
#' @description 
...
# if (verbose >= 2) {cat(report("  Creating draft lookup table\n"))}
  ...
  if (verbose >= 2) {cat(report("  Writing draft lookup table to",outfilespec,". Edit before use\n"))}
