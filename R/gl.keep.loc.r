#' @name gl.keep.loc
#' @title Remove all but the specified loci from a genelight \{adegenet\} object
#' @description 
...
# FUNCTION SPECIFIC ERROR CHECKING
  ...
  if (!is.null(loc.list) && !is.null(first)){
    ...
    if (verbose >= 2){
      ...
      cat(warn("  Warning: Need to specify either a range of loci to keep, or specific loci to keep\n"))
  }
...
# DO THE JOB
  ...
  if (length(loc.list) == 0) {
    cat(warn("  Warning: no loci listed to keep! Genlight object returned unchanged\n"))
    x2 <- x
  } else {
