...
gl.sims <- function(file_var=system.file('extdata','sim_variables.csv',package='dartR'),
  ...
  cat(important("  The description of the variables and their range of values 
   ...
   vars_assign <- unlist(unname(mapply(paste, sim_vars$variable, "<-", 
  ...
  ##### DISPERSAL ######
      ...
      if (dispersal == TRUE) {
        ...
        if(dispersal_type=="circle"){
          dispersal_pairs <- as.data.frame(rbind(cbind(pops_vector,c(pops_vector[-1],pops_vector[1])),
  ...
  ##### SAMPLING NEXT GENERATION ########
      ...
      if (any(test_extinction==TRUE)) {
        ...
        final_res[[iteration]][[count_store]] <- store(p_vector = pops_vector,
      ...
      if (selection == TRUE & natural_selection_model == "relative") {
        ...
        pop_list <- lapply(pops_vector,function(x){
          ...
          rbind(males_pop[sample(row.names(males_pop),
                                 size = (population_size / 2),
                                 prob = (males_pop$relative_fitness * 2)), ],
                females_pop[sample(row.names(females_pop),
                                   size = (population_size / 2),
