...
`CritSam` <- function(n,Dpos,alphalimit,pvaluetype) {
  ...
  for (i in 1:nrow(X)) {
    ...
    pval <- c(pval,HardyWeinberg::HWExact(X[i,],alternative="two.sided",pvaluetype=pvaluetype,verbose =F)$pval)
  }
...
`CritSam_Chi` <- function(n,Dpos,alphalimit,cc) {
  ...
  for (i in 1:nrow(X)) {
    fA <- c(fA,(2*X[i,1]+X[i,2])/(2*n))
    Ds <- c(Ds,HardyWeinberg::HWChisq(X[i,],cc=cc,verbose =FALSE)$D)
