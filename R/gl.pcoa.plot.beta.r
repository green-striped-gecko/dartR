...
#'
#' This script takes output from the ordination generated by gl.pcoa.beta() and plots the individuals classified by population.
#'
#' The factor scores are taken from the output of gl.pcoa.beta() -- an object of class pcoa -- and the population assignments are taken from
...
#'
#' Any pair of axes can be specified from the ordination, provided they are within the range of the nfactors value provided to gl.pcoa.beta(). Axes can be scaled to
...
#' pcoa<-gl.pcoa.beta(gl,nfactors=5)
#' gl.pcoa.plot.beta(pcoa, gl, ellipse=TRUE, p=0.99, labels="pop",hadjust=1.5, vadjust=1)
#' gl.pcoa.plot.beta(pcoa, gl, ellipse=TRUE, p=0.99, labels="pop",hadjust=1.5, vadjust=1, xaxis=1, yaxis=3)
#' 
#' df <- gl.pcoa.plot.beta(pcoa, gl, plot.out=FALSE)
#' df

gl.pcoa.plot.beta <- function(x, gl, scale=FALSE, ellipse=FALSE, p=0.95, labels="pop", hadjust=1.5, 
...
# STANDARD ERROR CHECKING
    ...
    if(class(x)!="pcoa" | class(gl)!="genlight") {
    cat("Fatal Error: pcoa and genlight objects required for x and gl parameters respectively!\n"); stop()
  }
  
# DO THE JOB
  
  # Create a dataframe to hold the required scores
    m <- cbind(x$vectors[,xaxis],x$vectors[,yaxis])
    df <- data.frame(m)
    
  # Convert the eigenvalues to percentages
    s <- sum(x$values$Eigenvalues[x$values$Eigenvalues >= 0])
    e <- round(x$values$Eigenvalues*100/s,1)
  ...
  if(plot.out) {
    ...
    df <- data.frame(id=indNames(gl), pop=popNames(gl), x$vectors)
    row.names(df) <- NULL
    return(df)
  }
