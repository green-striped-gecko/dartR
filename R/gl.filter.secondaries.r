...
# DO THE JOB
  ...
  if (verbose > 2) {cat(report("  Total number of SNP loci:",nLoc(x),"\n"))}
  
# Sort the genlight object on AlleleID (asc), RepAvg (desc), AvgPIC (desc) 
  if (method == "best") {
    if (verbose > 1){cat(report("  Selecting one SNP per sequence tag based on best RepAvg and AvgPIC\n"))}
    ...
    if (verbose > 1){cat(report("  Selecting one SNP per sequence tag at random\n"))}
