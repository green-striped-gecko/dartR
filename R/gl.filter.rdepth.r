...
#' @title Filter loci based on counts of sequence tags scored at a locus (read depth)
#' 
#' @description
...
#'  Deviation of Read Depth as stanard columns in their report.
#' 
...
# DO THE JOB
  ...
  if (verbose >= 2){cat(report("  Removing loci with rdepth <=",lower,"and >=",upper,"\n"))}
  ...
  if(save2tmp & plot.out){
    temp_plot <- tempfile(pattern = paste0("Plot",paste0(names(match.call()),"_",as.character(match.call()),collapse = "_"),"_"))
