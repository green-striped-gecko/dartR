...
gl.read.csv <- function(filename, 
  ...
  if (is.null(loc.metafile) & verbose>0){
    cat(warn("Warning: Locus metafile not provided, locus metrics will be 
        calculated where this is possible\n"))
  }
  
  if (is.null(ind.metafile) & verbose>0){
    cat(warn("Warning: Individual metafile not provided, pop set to 'A' for all individuals\n"))
  }
  ...
  if(nchar(test) > nrow(data)*ncol(data)) {
    if(verbose >=2){
      cat(report("Character data detected, assume genotypes are of the 
  ...
  if(!is.null(loc.metafile)) {
    ...
    cat(error("Fatal Error: mandatory AlleleID column absent 
  ...
  if (verbose >= 2){
    cat(report(paste(" Added or updated ",names(gl@other$loc.metrics),       "to the other$ind.metrics slot.\n")))
    }
  ...
  if(!is.null(ind.metafile)) {
    ...
    if (verbose >= 2){
      cat(report(paste(" Added ",names(gl@other$ind.metrics),
