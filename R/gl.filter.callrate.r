...
#' @title Filter loci or specimens in a genlight \{adegenet\} object based on call rate
#' @description 
#' SNP datasets generated by DArT have missing values primarily arising from failure to call a SNP because of 
#' a mutation at one or both of the the restriction enzyme recognition sites. The script gl.filter.callrate() will filter out the loci 
#' with call rates below a specified threshold.
#' 
...
#' Some individuals with a call rate initially greater than the nominated threshold, and so retained, may come 
#' to have a call rate lower than the threshold. If this is a problem, repeated iterations of this function 
#' will resolve the issue. This is done by setting mono.rm=TRUE and recursive=TRUE, or it can be done manually.
#'
...
# FUNCTION SPECIFIC ERROR CHECKING
  ...
  if (x@other$loc.metrics.flags$monomorphs == FALSE){
    if (verbose >= 2){
      cat(warn("  Warning: Data may include monomorphic loci in call rate calculations for filtering\n"))
    }  
  }
...
# FOR METHOD BASED ON LOCUS    
  ...
  if( method == "loc" ) {
    ...
    if (verbose >= 2) {cat(report("  Removing loci based on Call Rate, threshold =",threshold,"\n"))}
    n0 <- nLoc(x)
...
# FOR METHOD BASED ON INDIVIDUALS    
  ...
  if ( method == "ind" ) {
    ...
    # Determine starting number of loci and individuals
    if (verbose >= 2) {cat(report("  Removing individuals based on Call Rate, threshold =",threshold,"\n"))}
      n0 <- nInd(x)
#    if (verbose >= 3) {cat("Initial no. of individuals =", n0, "\n")}
    ...
    # Check that there are some individuals left
      if (sum(ind.call.rate >= threshold) == 0) {
        stop(error(paste("Maximum individual call rate =",max(ind.call.rate),". Nominated threshold of",threshold,"too stringent.\n No individuals remain.\n")))
      }
      if (!recursive) {
   ...
   if (verbose >= 2) {
     if (method == "ind"){
       ...
       if (!mono.rm) {
         ...
         if (!recursive) {cat(warn("  Warning: Some individuals with a Call Rate initially >=",threshold,"may have a final CallRate lower than",threshold,"when call rate is recalculated after removing resultant monomorphic loci\n"))}
       }   
     }
   }
