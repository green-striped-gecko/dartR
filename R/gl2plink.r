...
#' @param bed_file Whether create PLINK files .bed, .bim and .fam [default FALSE].
...
gl2plink <- function(x,
  ...
  if (snp_chr == "0") {
    ...
    if (chr_format == "numeric") {
      snp_temp$chrom <-
        as.numeric(unname(unlist(snp_temp[snp_chr])))
    }
    if (chr_format == "character") {
      snp_temp$chrom <-
        as.character(unname(unlist(snp_temp[snp_chr])))
    }
  }
  ...
  if (snp_pos == "0") {
    ...
    snp_temp$snp_pos <-
      as.numeric(unname(unlist(snp_temp[snp_pos])))
  }
  ...
  if (sex_code != "unknown") {
    sex_code[sex_code == "female"] <- "2"
    sex_code[sex_code == "male"] <- "1"
    sex_code[sex_code == "unknown"] <- "0"
  }
  ...
  if (bed_file) {
    ...
    make_plink <- function(plink.path,
